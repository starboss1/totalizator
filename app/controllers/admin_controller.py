...
def draws():
    ...
    if form.validate_on_submit():
        draw = db_queries.create_draw(draw_name=form.name.data)
        if form.add_random_events.data:
            db_queries.add_random_events_to_draw(draw=draw, date=form.date.data)
            flash(f"Random events added to draw", "success")
        flash(f"Draw #{draw.id} created successfully", "success")

    draws_list = db_queries.get_all_draws()
...
def draw_edit(draw_id):
    ...
    if form.validate_on_submit():
        try:
            event = db_queries.create_event(event_name=form.name.data, event_datetime=form.date.data, draw=draw)
...
def test_distribute(draw_id):
    db_queries._distribute_pool(db_queries.get_draw_by_id(draw_id))
